 ...
 */
 ...
 const possibleSkateDays = (weatherData) => {
  let lowTemperatureIndices = [];
  for (let i = 0; i < weatherData.length; i++) {
    if (weatherData[i].highestTemperature <= 0) {
      lowTemperatureIndices.push(i);
    }
  }
  for (let i = 0; i < lowTemperatureIndices.length; i++) {
    if (lowTemperatureIndices[i + 1] - lowTemperatureIndices[i] === 1) {
      if (i === 5) {
        const skateDays = lowTemperatureIndices
          .map((index) => weatherData[index].date)
          .slice(i);
        return skateDays;
      }
    } else {
